...
function Invoke-GateStep {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host ""
        Write-Host "Smart Editor Phase W4 gate FAILED at: $Name"
        exit $LASTEXITCODE
    }
}

Write-Host "EduSheet Smart Editor Phase W4 production gate"
Write-Host "Scope: W1-W3 regressions, Smart/Full tools, command palette, slash commands, reusable blocks, passive academic suggestions"
...
Invoke-GateStep -Name 'Smart Editor W3 Math + Geometry regression' -Action {
    flutter test test/features/smart_editor/smart_editor_w3_test.dart
}

Invoke-GateStep -Name 'Smart Editor W4 Smart/WPS experience certification' -Action {
    flutter test test/features/smart_editor/smart_editor_w4_test.dart
}
...
Invoke-GateStep -Name 'Geometry editor regression' -Action {
...
Write-Host ""
Write-Host "Smart Editor Phase W4 gate PASSED"
