...
try {
  Write-Host 'EduSheet Professional Formatting Phase 4B gate' -ForegroundColor Cyan
  Write-Host 'Baseline: Phase 4A QA Fix 2 / +337 green'
  ...
  Write-Host '3/6 Phase 4B focused tests'
  $focusedTests = @(
    @('test/features/paper_composer/word_shapes_phase4b_test.dart', 'Phase 4B shape model/service tests'),
    @('test/features/paper_composer/word_shape_preview_phase4b_test.dart', 'Phase 4B shape preview tests'),
    @('test/features/paper_composer/word_direct_authoring_phase4a_test.dart', 'Phase 4A direct authoring regression'),
    @('test/features/paper_composer/dual_editor_mode_test.dart', 'Dual editor regression'),
    @('test/features/pdf/office_export_services_test.dart', 'PDF/DOCX export parity tests'),
    @('test/release/professional_formatting_phase4b_gate_test.dart', 'Phase 4B release gate')
  )
  ...
  Write-Host 'Verify Shapes on narrow Android and resized Windows:' -ForegroundColor Yellow
  Write-Host 'drag/resize, Square left/right, Top & bottom, Behind/In front, layer buttons,' -ForegroundColor Yellow
  Write-Host 'Text Box/Callout editing, Preview parity, PDF, DOCX and Smart/Word round-trip.' -ForegroundColor Yellow
  Write-Host ''
  Write-Host 'EduSheet Phase 4B automated gate PASSED.' -ForegroundColor Green
} catch {
  Write-Host ''
  Write-Host 'EduSheet Phase 4B gate FAILED.' -ForegroundColor Red
