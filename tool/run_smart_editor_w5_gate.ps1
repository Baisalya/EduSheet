...
Write-Host "EduSheet Smart Editor Phase W5 production gate"
Write-Host "Scope: W1-W4 regressions, DOCX import/export round-trip safety, PDF export, Word parser/writer regressions, Math/Geometry interoperability"
...
function Run-Step([string]$Name, [scriptblock]$Action) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host ""
        Write-Host "Smart Editor Phase W5 gate FAILED at: $Name" -ForegroundColor Red
        exit $LASTEXITCODE
    }
}
...
Run-Step "W5 DOCX/PDF interoperability" { flutter test test/features/smart_editor/smart_editor_w5_test.dart }
...
Write-Host ""
Write-Host "Smart Editor Phase W5 production gate PASSED" -ForegroundColor Green
