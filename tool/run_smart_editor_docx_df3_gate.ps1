...
function Invoke-Step([string]$Name, [scriptblock]$Action) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host ""
        Write-Host "Smart Editor DOCX DF3 gate FAILED at: $Name" -ForegroundColor Red
        exit $LASTEXITCODE
    }
}

Write-Host "EduSheet Smart Editor DOCX DF3 gate" -ForegroundColor Green
Write-Host "Scope: structured editable Word tables/text boxes/images, preserved cell geometry, safe table mutations, DF2/DF1 regressions"
...
Write-Host ""
Write-Host "Smart Editor DOCX DF3 gate PASSED" -ForegroundColor Green
