...
function Fail-GateStep {
    ...
    Write-Host "`nWord Converter Phase 5 gate FAILED at: $Name" -ForegroundColor Red
    exit $ExitCode
}
...
Write-Host 'EduSheet Word Converter Phase 5 release gate'
Write-Host 'Scope: Windows Preserve Appearance parity + per-page appearance geometry'
...
Invoke-GateStep -Name 'Windows PDF renderer contract' -Command {
    flutter test test/features/word_converter/windows_pdf_renderer_contract_test.dart
}
...
Invoke-GateStep -Name 'Windows native PDF renderer compile' -Command {
    flutter build windows --debug --no-pub
}

Write-Host "`nWord Converter Phase 5 gate PASSED" -ForegroundColor Green
exit 0
